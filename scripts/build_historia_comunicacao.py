...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .plano-banner { background: #ffe600; color: #17384d; font-weight: 800; font-size: 21px; text-align: center; padding: 14px; border-radius: 10px; margin-bottom: 18px; }
  .plano-item { display: flex; align-items: center; justify-content: space-between; gap: 16px; background: white; border-radius: 10px; padding: 16px 22px; box-shadow: 0 2px 8px rgba(0,0,0,.06); margin-bottom: 10px; font-size: 18px; color: #222; font-weight: 600; }
  .plano-subitems { margin: 0 0 16px 40px; padding-left: 18px; border-left: 3px dashed #d5dde3; }
  .plano-sub { display: flex; align-items: center; justify-content: space-between; gap: 16px; background: #fafbfc; border: 1px solid #e5e8eb; border-radius: 8px; padding: 10px 18px; margin-bottom: 8px; font-size: 15px; color: #444; }
  .status-badge { display: inline-flex; align-items: center; gap: 4px; padding: 4px 12px; border-radius: 14px; font-size: 13px; font-weight: 700; white-space: nowrap; flex-shrink: 0; }
  .status-badge.done { background: #e2f6ec; color: #00723a; }
  .status-badge.wip { background: #fff4cf; color: #9a6b00; }
  .plano-note { font-size: 14px; color: #666; font-style: italic; margin: -4px 0 8px 0; }
  .plano-links { display: flex; flex-wrap: wrap; gap: 8px; margin: 0 0 16px 0; }
  .plano-link { background: #eef8ff; color: #009ee3; padding: 4px 14px; border-radius: 14px; font-size: 13px; font-weight: 700; text-decoration: none; }
  .plano-link:hover { background: #d9f0fc; }
...
const MES_LABELS = {
  PT: { '01':'Jan','02':'Fev','03':'Mar','04':'Abr','05':'Mai','06':'Jun','07':'Jul','08':'Ago','09':'Set' },
  ES: { '01':'Ene','02':'Feb','03':'Mar','04':'Abr','05':'May','06':'Jun','07':'Jul','08':'Ago','09':'Sep' }
};
...
const I18N = {
  PT: {
    ...
    s6: {
      kicker: 'Pra Fechar',
      h1: 'O que estamos fazendo pra reverter isso',
      lead: 'Plano de ação em andamento -- revisão de configuração das campanhas, novos touchpoints e canais complementares.',
      planoTitle: 'Plano de Ação',
      doneLabel: 'Feito',
      wipLabel: 'Em andamento',
      items: [
        { title: 'Revisão da configuração das campanhas', status: 'wip', subitems: [
          'Correção da campanha D7 para D10: ajuste na query',
          'Correção da campanha D1: ajuste na query e revisão de configuração da journey (SWITCH)',
          'Ajuste na reentrada de usuários no fluxo de 30D para 25D',
          'Revisão IE Eldorado: 7 dias de uso do cupom após emissão de TC'
        ]},
        { title: 'Inclusão de novos touchpoints como alavanca de conversão (e-mail, D4) -- jul/26', status: 'done',
          alert: 'Implementado, mas ainda sem resultado visível na conversão -- depende da correção da campanha D1 (ajuste de query + configuração da journey SWITCH), que segue em andamento.' },
        { title: 'Ajuste das campanhas para entrada diária de usuários na jornada -- ago/26', status: 'done' },
        { title: 'Ativação do POM como canal complementar de conversão -- aquisição: set/26, engajamento: ago/26', status: 'wip',
          note: 'Pra engajados, já estamos online no TikTok:',
          links: [
            'https://v-ttam.tiktok.com/s/ZSVp8j7Qx/',
            'https://v-ttam.tiktok.com/s/ZSVp8ddAC/',
            'https://v-ttam.tiktok.com/s/ZSVp8Pw85/',
            'https://v-ttam.tiktok.com/s/ZSVp8f8dn/',
            'https://v-ttam.tiktok.com/s/ZSVp8h6eP/',
            'https://v-ttam.tiktok.com/s/ZSVp8U66b/',
            'https://v-ttam.tiktok.com/s/ZSVp8ysY4/'
          ] }
      ],
      videoLabel: 'Vídeo'
    },
  ...
  ES: {
    ...
    s6: {
      kicker: 'Para Cerrar',
      h1: 'Qué estamos haciendo para revertir esto',
      lead: 'Plan de acción en curso -- revisión de configuración de las campañas, nuevos touchpoints y canales complementarios.',
      planoTitle: 'Plan de Acción',
      doneLabel: 'Hecho',
      wipLabel: 'En curso',
      items: [
        { title: 'Revisión de la configuración de las campañas', status: 'wip', subitems: [
          'Corrección de la campaña D7 a D10: ajuste en la query',
          'Corrección de la campaña D1: ajuste en la query y revisión de configuración de la journey (SWITCH)',
          'Ajuste en la reentrada de usuarios en el flujo de 30D a 25D',
          'Revisión IE Eldorado: 7 días de uso del cupón después de la emisión de TC'
        ]},
        { title: 'Inclusión de nuevos touchpoints como palanca de conversión (email, D4) -- jul/26', status: 'done',
          alert: 'Implementado, pero todavía sin resultado visible en conversión -- depende de la corrección de la campaña D1 (ajuste de query + configuración de la journey SWITCH), que sigue en curso.' },
        { title: 'Ajuste de las campañas para entrada diaria de usuarios en la jornada -- ago/26', status: 'done' },
        { title: 'Activación del POM como canal complementario de conversión -- adquisición: set/26, engagement: ago/26', status: 'wip',
          note: 'Para engagement, ya estamos online en TikTok:',
          links: [
            'https://v-ttam.tiktok.com/s/ZSVp8j7Qx/',
            'https://v-ttam.tiktok.com/s/ZSVp8ddAC/',
            'https://v-ttam.tiktok.com/s/ZSVp8Pw85/',
...
const avgRangeLinePlugin = {
  ...
  afterDatasetsDraw(chart, args, opts) {
    ...
    opts.ranges.forEach(r => {
      ...
      const labelY = r.labelAbove ? y - 14 : y + 14;
...
function slide6() {
  const t = L().s6;
  const statusBadge = status => `<span class="status-badge ${status}">${status === 'done' ? '✅ ' + t.doneLabel : '🚧 ' + t.wipLabel}</span>`;
  const itemsHtml = t.items.map(item => {
    const sub = item.subitems ? `
      <div class="plano-subitems">
        ${item.subitems.map(s => `<div class="plano-sub"><span>${s}</span>${statusBadge('wip')}</div>`).join('')}
      </div>` : '';
    const alert = item.alert ? `<div class="note-box" style="margin-bottom:18px;">${item.alert}</div>` : '';
    const note = item.note ? `<div class="plano-note">${item.note}</div>` : '';
    const links = item.links ? `<div class="plano-links">${item.links.map((url, i) => `<a class="plano-link" href="${url}" target="_blank" rel="noopener">${t.videoLabel} ${i + 1}</a>`).join('')}</div>` : '';
    return `
  ...
  <div class="slide" id="slide-6">
    ...
    <h1>${t.h1}</h1>
    <div class="lead">${t.lead}</div>
    <div class="plano-banner">${t.planoTitle}</div>
    ${itemsHtml}
  </div>`;
}
